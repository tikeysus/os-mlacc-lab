...
This project uses lightweight local checks through `make verify` and a Git pre-commit hook.
...
- C code: `-Wall -Wextra -Werror`, optional `clang-format` when installed, `snake_case` function names, function length under 80 lines, nesting depth at 4 or less, and source/header files under 500 lines.
